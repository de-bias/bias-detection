...
if (dfd == "twitter") {
  df_bias <- read.csv(paste0(wd, "/data/outputs/twitter/monthly/populations/active-population-bias/active_population_bias_lad_w_db.csv")) %>% rename("geography.code" = "code") %>% dplyr::filter(name != "City of London")
  } else if (dfd == "fb_tts") {
  df_bias <- read.csv(paste0(wd, "/data/outputs/fb/census-month/populations/tts/active-population-bias/active_population_bias_lad_w_db.csv")) %>% rename("geography.code" = "code")
  } else if (dfd == "fb_stt") {
  df_bias <- read.csv(paste0(wd, "/data/outputs/fb/census-month/populations/stt/active-population-bias/active_population_bias_lad_w_db.csvv")) %>% rename("geography.code" = "code")
  }
...
if ("bias_w" %in% names(df_bias)) {
  df_var_lbl[46, ] <- list("bias_w", "spatial_lag_bias", "Spatial Lag", 
                      "spatial_lag_bias", "spatial_lag_bias", "spatial_lag_bias")
}
```
...
sv_importance(shp, kind = "bee", viridis_args = list(option = "viridis") ) +
...
```{r}
# external importance scores for ordering from importance_matrix
external_order_t15 <- as.data.frame(importance_matrix$Feature) %>% 
  rename(selected_feature = "importance_matrix$Feature") %>% 
  head(n=15)

#sv_importance(shp, show_numbers = TRUE)
feature_lbl <- sv_importance(shp, kind = "bee", max_display = 30L) %>% 
  .$data %>% 
  distinct(.$feature) %>% 
  rename(labels = ".$feature") %>% 
  # left_join(., df_var_lbl, join_by(labels == var_label0)) %>% 
  # left_join(external_order_t15, ., join_by(selected_feature == labels))

sv_importance(shp, kind = "beeswarm", viridis_args = list(option = "viridis") ) +
  scale_y_discrete( labels = rev(feature_lbl$var_label2)) +
  theme_plot_tufte() 
...
sv_importance(shp, kind = "beeswarm", 
              max_display = 15L,
              viridis_args = list(option = "viridis") ) +
  scale_y_discrete(limits = external_order_t15$selected_feature) +
  theme_plot_tufte() 
```
...
```{r}
# Return the SHAP values and ranked features by mean|SHAP|
shap_values_full <- shap.values(xgb_model = xgb.fit.final_all, X_train = as.matrix(input_df[, 4:ncol(input_df)-1]))
