...
```{r}
dfd <- "mapp2"
include_spatial_lag_covariates <- TRUE
lag_tag <- ifelse(include_spatial_lag_covariates, "lagcov", "nolagcov")
output_tag <- paste0("_lad_holdout_", lag_tag)
```
...
if (isTRUE(include_spatial_lag_covariates)) {
  # Ensure the same spatial order as polygons when creating lagged covariates
...
if (dfd == "twitter"){
  write_csv(results, paste0(wd, "/data/outputs/", dfd, "/monthly/populations/explain-bias/hypergrid", output_tag, ".csv"))
  } else if (dfd == "fb_tts") {
  write_csv(results, paste0(wd, "/data/outputs/fb/census-month/populations/tts/explain-bias/hypergrid", output_tag, ".csv"))
  } else if (dfd == "fb_stt") {
  write_csv(results, paste0(wd, "/data/outputs/fb/census-month/populations/stt/explain-bias/hypergrid", output_tag, ".csv"))
  } else if (dfd == "mapp1") {
  write_csv(results, paste0(wd, "/data/outputs/mapp1/census-month/populations/explain-bias/hypergrid", output_tag, ".csv"))
  } else if (dfd == "mapp2") {
  write_csv(results, paste0(wd, "/data/outputs/mapp2/census-month/populations/explain-bias/hypergrid", output_tag, ".csv"))
  }
...
if (dfd == "twitter"){
  hyper_grid <- read_csv(paste0(wd, "/data/outputs/", dfd, "/monthly/populations/explain-bias/hypergrid", output_tag, ".csv"))
  } else if (dfd == "fb_tts") {
  hyper_grid <- read_csv(paste0(wd, "/data/outputs/fb/census-month/populations/tts/explain-bias/hypergrid", output_tag, ".csv"))
  } else if (dfd == "fb_stt") {
  hyper_grid <- read_csv(paste0(wd, "/data/outputs/fb/census-month/populations/stt/explain-bias/hypergrid", output_tag, ".csv"))  
  } else if (dfd == "mapp1") {
  hyper_grid <- read_csv(paste0(wd, "/data/outputs/mapp1/census-month/populations/explain-bias/hypergrid", output_tag, ".csv"))  
  } else if (dfd == "mapp2") {
  hyper_grid <- read_csv(paste0(wd, "/data/outputs/mapp2/census-month/populations/explain-bias/hypergrid", output_tag, ".csv"))  
  }
...
if (dfd == "twitter"){
  saveRDS(xgb.fit.final, paste0(wd, "/data/outputs/", dfd, "/monthly/populations/explain-bias/xgb-fit-final", output_tag, ".rds"))
  } else if (dfd == "fb_tts") {
  saveRDS(xgb.fit.final, paste0(wd, "/data/outputs/fb/census-month/populations/tts/explain-bias/xgb-fit-final", output_tag, ".rds"))
  } else if (dfd == "fb_stt") {
  saveRDS(xgb.fit.final, paste0(wd, "/data/outputs/fb/census-month/populations/stt/explain-bias/xgb-fit-final", output_tag, ".rds"))
  } else if (dfd == "mapp1") {
  saveRDS(xgb.fit.final, paste0(wd, "/data/outputs/mapp1/census-month/populations/explain-bias/xgb-fit-final", output_tag, ".rds"))
  } else if (dfd == "mapp2") {
  saveRDS(xgb.fit.final, paste0(wd, "/data/outputs/mapp2/census-month/populations/explain-bias/xgb-fit-final", output_tag, ".rds"))
  }
```
...
write_csv(optimal_pars[1, ], paste0("../outputs/preliminary/",dfd,"/explain-bias/optimal_parameters", output_tag, ".csv"))
```
...
```{r}
# Read final model (same path logic as saveRDS block)
if (dfd == "twitter"){
  xgb.fit.final <- readRDS(paste0(wd, "/data/outputs/", dfd, "/monthly/populations/explain-bias/xgb-fit-final", output_tag, ".rds"))
} else if (dfd == "fb_tts") {
  xgb.fit.final <- readRDS(paste0(wd, "/data/outputs/fb/census-month/populations/tts/explain-bias/xgb-fit-final", output_tag, ".rds"))
} else if (dfd == "fb_stt") {
  xgb.fit.final <- readRDS(paste0(wd, "/data/outputs/fb/census-month/populations/stt/explain-bias/xgb-fit-final", output_tag, ".rds"))
} else if (dfd == "mapp1") {
  xgb.fit.final <- readRDS(paste0(wd, "/data/outputs/mapp1/census-month/populations/explain-bias/xgb-fit-final", output_tag, ".rds"))
} else if (dfd == "mapp2") {
  xgb.fit.final <- readRDS(paste0(wd, "/data/outputs/mapp2/census-month/populations/explain-bias/xgb-fit-final", output_tag, ".rds"))
}
```
...
write_csv(summary_table, paste0("../outputs/preliminary/",dfd,"/explain-bias/model-performance", output_tag, ".csv"))
```

### Residual spatial autocorrelation (Moran's I)

```{r}
# Predict on full analysis sample and compute residuals
...
# One residual per area (safe if repeated rows ever appear)
resid_area <- input_df %>%
  dplyr::group_by(geography.code) %>%
  dplyr::summarise(model_residual = mean(model_residual, na.rm = TRUE), .groups = "drop")

# Use already imported LAD boundaries and create Queen contiguity neighbours
lda_sdf_resid <- lda_sdf %>%
  dplyr::filter(!grepl("^[SN]", code))

# Align residuals to polygon order
resid_aligned <- lda_sdf_resid %>%
  sf::st_set_geometry(NULL) %>%
  dplyr::transmute(geography.code = code) %>%
  dplyr::left_join(resid_area, by = "geography.code")

nb_q_resid <- spdep::poly2nb(lda_sdf_resid, queen = TRUE)
keep <- is.finite(resid_aligned$model_residual)
nb_sub <- spdep::subset.nb(nb_q_resid, subset = keep)
...
write_csv(model_residual_moran, paste0("../outputs/preliminary/", dfd, "/explain-bias/model-residual-moran", output_tag, ".csv"))
...
```{r}
png(paste0("../outputs/preliminary/",dfd,"/explain-bias/feature-importance_v2", output_tag, ".png"), units="in", width=8, height=10, res=300)
   last_plot()
dev.off()
```
...
```{r}
png(paste0("../outputs/preliminary/",dfd,"/explain-bias/feature-importance", output_tag, ".png"), units="in", width=8, height=10, res=300)
  last_plot()
dev.off()
```
...
write_csv(fi_data, paste0("../outputs/preliminary/",dfd,"/explain-bias/feature-importance-data", output_tag, ".csv"))
```
...
```{r}
png(paste0("../outputs/preliminary/",dfd,"/explain-bias/shap-plot-bias", output_tag, ".png"), units="in", width=8, height=10, res=300)
  last_plot()
dev.off()
```
...
```{r}
png(paste0("../outputs/preliminary/",dfd,"/explain-bias/shap-dependence", output_tag, ".png"), units="in", width=8, height=10, res=300)
  last_plot()
dev.off()
```
...
```{r}
png(paste0("../outputs/preliminary/",dfd,"/explain-bias/heatmap_xxldas", output_tag, ".png"), units="in", width=8, height=10, res=300, bg = "transparent")
  last_plot()
dev.off()
```
