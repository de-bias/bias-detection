...
```{r}
df <- merge(df_standardised, df_bias[, c("geography.code", "bias")], by = "geography.code")
```
...
selected_features <- importance_matrix[order(importance_matrix$Gain, decreasing = TRUE)] %>%
  head(., n=3) %>% 
  distinct(Feature) 

selected_features_lbl <- importance_matrix[order(importance_matrix$Gain, decreasing = TRUE)] %>%
  head(., n=3) %>% 
  distinct(Feature) %>% 
  rename(labels = "Feature") %>% 
  left_join(., df_var_lbl, join_by(labels == var_label0))
...
sv_dependence(shp, v = unique(selected_features$Feature), alpha = 0.8 ) +
  plot_layout(ncol = 1)
```
...
shap.plot.dependence(data_long = shap_long, x = 'per_hh_no_centralheat', 
                     color_feature = 'per_hh_no_centralheat', 
                     size0 = 2)
```

```{r}
left_join(shap_long, head(feature_lbl, n = 4), 
                                join_by(variable == labels) ) %>% 
  dplyr::filter(!is.na(var_label2)) %>% 
  ggplot(aes( x = rfvalue, y = value, colour = rfvalue)) +
  geom_smooth(colour = "gray95", span = 0.3) +
  geom_point(alpha = 0.7, size = 3) +
  scale_colour_viridis(labels = label_number(scale_cut = cut_short_scale()),
  ...
  facet_wrap(~ var_label2, scales = "free", ncol = 2, strip.position = "top") +
  labs(y = "Shape value",
       x = "Feature value") +
  theme_plot_tufte() +
  theme(
...
selected_data <- left_join(shap_long, head(feature_lbl, n = 4), 
                                join_by(variable == labels) ) %>% 
  dplyr::filter(!is.na(var_label2))

# Create a custom plot function that applies per-facet scaling
plot_faceted_with_individual_color_scale <- function(data) {
  # Split the data by facet variable
  plots <- data %>% 
    group_by(var_label2) %>%
    group_map(~{
      ggplot(.x, aes(x = rfvalue, y = value, color = rfvalue)) +
        geom_smooth(colour = "gray95", span = 0.3) +
        geom_point(alpha = 0.7, size = 3) +
        scale_y_continuous(
        ...
        scale_colour_viridis_c(option = "D") +  # Apply Viridis color scale
        labs(y = "Shape value", 
             x = "Feature value") +
                annotate("text", x = Inf, y = Inf, label = unique(.x$var_label2), 
                 hjust = 1.1, vjust = 2, size = 5, fontface = "bold") + # Add title as annotation +
        theme_plot_tufte() +
        theme(
          strip.text = element_text(size = 14),
          legend.position = "none",
  ...
  combined_plot <- wrap_plots(plots, nrow = 2) # Arrange in 2 rows
  return(combined_plot)
}

# Use the custom function to create the plot
plot_faceted_with_individual_color_scale(selected_data)
```

```{r}
# Custom plot function with individual color scales per facet
plot_faceted_with_individual_color_scale <- function(data) {
  # Create a list to store each plot
  plot_list <- list()
  
  # Generate individual plots with separate color scales
  data %>%
    group_by(var_label2) %>%
    group_walk(~{
      # Generate the individual plot with viridis scale based on each subset
      p <- ggplot(.x, aes(x = rfvalue, y = value, color = rfvalue)) +
        geom_smooth(color = "gray95", span = 0.3) +
        geom_point(alpha = 0.7, size = 3) +
        scale_y_continuous(
          labels = scales::label_number(scale_cut = scales::cut_short_scale(), accuracy = 0.001)
        ) +
        scale_x_continuous(
          labels = scales::label_number(scale_cut = scales::cut_short_scale())
        ) +
        scale_colour_viridis_c(option = "D") +  # Apply Viridis color scale to each plot individually
        labs(y = "Shape value", x = "Feature value") +
        ggtitle(unique(.x$var_label2)) +  # Use var_label2 as the title for each plot
        theme_minimal() +
        theme(
...
selected_variables <- c("per_hh_no_centralheat", "per_NS_SeC_L13_routine", 
                        "per_ukborn", "per_hh_no_car")
...
for (var in selected_variables) {
  ...
  p <- ggplot(data_subset, aes(x = rfvalue, y = value, color = rfvalue)) +
    ...
    labs(title = var, x = "Feature value (rfvalue)", y = "Dependent value") +
    theme_minimal() +
    theme(
