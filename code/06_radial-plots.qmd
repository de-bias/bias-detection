...
df_meta <- tribble(
  ...
  "socioeconomic", "per_no_qualifications",                                3,
...
df_raw  <- df_raw   %>%
  mutate(variable = variable %>%
           str_replace(regex("^\\s*SEC\\s*-\\s*", ignore_case = TRUE), "") %>%  # strip leading 'SEC -'
           str_squish() %>%
           str_to_sentence()                                                   # e.g., "lower supervisory" -> "Lower supervisory"
         )

replacements <- c("Severe disability (%)" = "Disability (%)",
                  "Not deprived (% hh)" = "Not deprived",
                  "No central heating (% hh)" = "No central heating")
df_raw  <- df_raw   %>%
