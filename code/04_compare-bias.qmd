...
ggplot(data, aes(x = x, y = y)) +
  ...
  labs(x = "Twitter (bias by LAD and month)",               # Label for the x-axis
       y = "FB tts (bias by LAD and month)") +              # Label for the y-axis
  theme_tufte2() +
...
ggsave(paste0(wd_local, "/bias-detection/outputs/preliminary/summary-figs/size-bias/scatter-plot/fb_ts_census_month_vs_twitter.png"), bg="white")
```
```{r}
paste0(wd_local, "/bias-detection/outputs/preliminary/summary-figs/size-bias/scatter-plot/fb_ts_census_month_vs_twitter.png")
```
...
ggplot(data, aes(x = x, y = y)) +
  geom_point(shape = 1, size = 1) +                        
  labs(x = "FB tts (bias by LAD and month)",               
       y = "FB stt (bias by LAD and month)") +             
  theme_tufte2() +
  annotate("text", x = 0.87, y = 0.9955, 
           label = paste("Spearman Correlation: ", round(spearman_corr, 2),
                         "\nP-value: ", format(spearman_pvalue, digits = 3)), 
           family = "robotocondensed", hjust = 0, vjust = 1, size = 4, color = "blue")
```
