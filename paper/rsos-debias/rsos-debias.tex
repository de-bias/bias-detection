...
We propose a systematic framework to measure and explain biases in population count data derived from multiple sources of mobile phone apps data. We focus on aggregated population counts as a privacy-preserving and ethically appropriate representation of digital mobility data. The analysis is applied on data collected during March 2021 to ensure comparability with recent the most census in the area of study. Figure 1 provides an overview of the methodological workflow, which includes data acquisition, bias estimation using a coverage indicator, and bias explanation through spatial modeling with contextual covariates. This integrated approach allows us to evaluate the representativeness of different data sources.

\subsection{Data (CC)}\label{data-cc}

To evaluate the extent of bias in digital population data, we assemble and compare four mobile-derived datasets representing both single- and multi-app data sources. These datasets differ in terms of their data generation mechanisms, geographic and temporal resolution, population coverage, and underlying user base. Facebook and Twitter/X data represent single-application platforms, while Locomizer and another European provider aggregate data from multiple mobile apps. Table 1 summarises the main characteristics of each dataset, including the source type, spatial coverage, temporal resolution, and known strengths and limitations. These differences are critical to understanding how data collection methods shape the representativeness of digital trace data. The following subsections describe each data source in detail.

\begin{figure}
\centering
\includegraphics[width=5.20833in,height=1.77083in]{figures/table-data-source.png}
\caption{Supplementary Table 1. Summary description of data sources used in the article, including key features.}
\end{figure}
