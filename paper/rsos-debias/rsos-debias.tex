...
Efforts have been made to measure and assess biases in digital nontraditional data sources.
Existing analyses typically measure the extent of bias measuring the difference in the representation of digital data forms and census data. To estimate the representation of digital data sources, the penetration rate is computed as the share of active users over the resident population.
Existing analyses have thus been able to established systematic gender, age and socio-economic biases in digital data obtained via API (or Application Programming Interface) from social media platforms, such as Facebook and Twitter/X.
However, this approach requires information on the demographic and socio-economic attributes of the collected sample, but
these attributes are rarely available from mobile phone network operators.
In practice, they are generally unavailable for MPD.

A key limitation to address
