...
<!-- Comments on the table below: Change from "Facebook Population" to "Facebook". Add a footenote to explain what "(pre-processed)" means.Change from "8-hour windows" to "8-hour window". For multi-app1, change from "Open access via GitHub (pre-processed)". Only preserve "(pre-processed)" from one of the columns, either from the form of collection or access method. -->
...
<!-- Replace the link to GitHub with the one for Zenodo which is now the official link to download the data -->
...
<!-- indicate what statistic this is-->
...
<!-- Provide the section of the SM in which this is reported -->
...
[@lohr2021]. Yet, representativeness of survey data can only be guaranteed with
...
<!-- The reference letters for each column should be added to the Figure -->
...
<!-- I see two keey issues with this statement: (1) are we referring to the range of the distribution per se, or the spatial distribution? The spatial distribution is relatively random and the range of bias scores is large for social media data than it is for multi-apps. That should be a main finding; (2) not sure the histograms if the best reference point to see geographic variation. I gather that some of these points are discussed below. But I think we need more careful and precise language to make the point in currently contained in this paragraph-->
For
...
<!-- This may sound misleading. I would reframe. Twitter/X shows probably the largest range of scores: the lowest and highest across the four datasets. True is that Twitter/X shows a higher concentration of bias coverage at the higher end. -->
...
locations detected. 
<!-- If easy to include, would provide the share of home locations detected in C of London across the dataset -->
...
<!-- The statement that follows is not supported by our analysis. Twitter/X has the **lowest** scores of bias - unless I am misreading. -->
...
distributions. 
<!-- Based on the range of bias scores, I would say that biases are distributed across a smaller range, hence more limited overall bias, no? -->
...
<!-- This last sentence needs to be elaborated if it's to make a point. -->
